...
public:
  ...
  void LMA(double tolerance = 1e-5, int max_iter = 10000, double lambdaUp = 10, double lambdaDown = 10) {
    ...
    for (int i = 0; i < p; ++i)  error[i] = sqrt(var * CoVar(i + 1, i + 1)); // standard error
...
private:
  ...
  Matrix GradFitFunction(const double x, const std::vector<double> &par) {
    ...
    grad(1,1) = 1/(1 + exp_term); // derivative with respect to A
    grad(2,1) = A *  exp_term / (R * (1+ exp_term) * (1 + exp_term)); // derivative with respect to T
    grad(3,1) = A * (x - T) * exp_term / (R * R * (1 + exp_term) * (1 + exp_term)); // derivative with respect to R
  ...
  int NonLinearRegression(double lambdaUp = 10, double lambdaDown = 10) {
    ...
    SSR = (Transpose(dY) * dY)(1, 1); // compute initial SSR

    Matrix dpar = CoVar * JtY; // p x 1 matrix for

    std::vector<double> par_new = par; // new parameters
    for (int i = 0; i < p; ++i)  par_new[i] += dpar(i+1, 1); // update parameters
