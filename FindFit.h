...
private:
  ...
  Matrix0 CoVar; // this is covariance / var, var = SSR / nDF
  ...
  Matrix0 GradFitFunction(const double x, const std::vector<double> &par) {
    Matrix0 grad(4,1);
  ...
  int NonLinearRegression(double lambdaUp = 10, double lambdaDown = 10) {
    ...
    Matrix0 J(n, p);  // n x p matrix for function values
    Matrix0 f(n ,1);  // n x 1 matrix for fitted values

    for( int i = 0; i < n; i++) {
      double x_val = valX[i];
      f(i, 0) = FitFunction(x_val, par);
      
      Matrix0 Frow = GradFitFunction(x_val, par); // p x 1 matrix for gradient
      for (int k = 0; k < p; k++)  J(i, k) = Frow(k, 0); 

    }

    Matrix0 Jt = Transpose(J);
    Matrix0 H = Jt * J;
    ...
    Matrix0 DI(H.GetNRow(), H.GetNCol()); // identity matrix
    for( int i = 0; i < H.GetNRow() ; i++) DI(i,i) = lambda ;

    Matrix0 Q = H + DI;
    try{
      CoVar = Inv(Q);
    }catch( Exception0 err){
      return -1; // Error in matrix inversion
    }

    Matrix0 dY = Y - f;
    Matrix0 JtY = Jt * dY; // p x 1 matrix for gradient of SSR

    SSR = (Transpose(dY) * dY)(0, 0); // compute initial SSR

    Matrix0 dpar = CoVar * JtY; // p x 1 matrix for
